...
class AIChatService {
  ...
  // -------------------------------------------------------------------------
  // Long-term memory (ai_user_memory)
  // -------------------------------------------------------------------------

  /** Fetch every durable fact the assistant has stored about a user. */
  async listMemories(userId) {
    if (!userId) return { memories: [], count: 0 }
    try {
      const response = await resilientFetch(
        `${API_BASE}/memory/${encodeURIComponent(userId)}`,
        { method: 'GET' },
        { retries: 1, timeout: 12000, label: 'ai/memory-list' }
      )
      if (!response.ok) {
        await throwAiHttpError(response, 'Could not load saved memories')
      }
      const data = await response.json()
      return {
      ...
      console.error('List AI memories error:', error)
      reportError(error)
      throw error
    }
  }

  /** Insert or overwrite a single memory (manual edit from Settings). */
  async upsertMemory(userId, { key, value, confidence = 1, source = 'explicit' }) {
    if (!userId || !key || !value) {
      throw new Error('userId, key, and value are required')
    }
    try {
      const response = await resilientFetch(
        `${API_BASE}/memory`,
        ...
        { retries: 1, timeout: 12000, label: 'ai/memory-upsert' }
      )
      if (!response.ok) {
        await throwAiHttpError(response, 'Could not save memory')
      }
      const data = await response.json()
      return data?.memory || null
    } catch (error) {
      console.error('Upsert AI memory error:', error)
      reportError(error)
      throw error
    }
  }
  ...
  async deleteMemory(userId, key) {
    if (!userId || !key) return { removed: 0 }
    try {
      const response = await resilientFetch(
        `${API_BASE}/memory/${encodeURIComponent(userId)}/${encodeURIComponent(key)}`,
        { method: 'DELETE' },
        { retries: 1, timeout: 12000, label: 'ai/memory-delete' }
      )
      if (!response.ok) {
        await throwAiHttpError(response, 'Could not delete memory')
      }
      const data = await response.json()
  ...
  async clearAllMemories(userId) {
    ...
    try {
      const response = await resilientFetch(
        `${API_BASE}/memory/${encodeURIComponent(userId)}`,
        { method: 'DELETE' },
        { retries: 1, timeout: 15000, label: 'ai/memory-clear' }
      )
      if (!response.ok) {
        await throwAiHttpError(response, 'Could not clear memories')
      }
      const data = await response.json()
      return { removed: Number(data?.removed) || 0 }
    } catch (error) {
