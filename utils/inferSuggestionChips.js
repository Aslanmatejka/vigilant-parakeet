...
export function inferChipsFromResponse(responseText, language = 'en') {
  ...
  const descriptionAsk = (
    /short description|add a description|describe the food|describe it|describing the|describing this|description for recipients|one-sentence description|one sentence description|one sentence about|one short sentence|sentence describing|few words about|tell me more about the food|tell me a bit about|tell me a bit more about|how would you describe|should know about the food|people should know|anything else about the food|note for recipients|listing description|put as the description|what should (the|i put).{0,20}description|what should the listing|listing say about|say about (this|the) food|sentence about the food|sentence for the (post|listing|description)|short blurb|jot a description|how is it packaged|how it'?s packed|how it is packed|how fresh|give me one short|short description of|in the carton|still in the carton|write one short sentence|what'?s included|whats included|say a little about|what'?s the food like|mention on the listing|details for the listing|help me with a listing description|describe what'?s in|\bdescription\??\s*$|descripci[oó]n corta|descripci[oó]n para|describe la comida/.test(t)
  ...
  if (postConfirm) {
    const photoEvidence = /photos received|got your photo|got the photo|got a photo|got the picture|got your picture|thanks for the photo|thanks for the picture|photo, thanks|with your photos|with photo|with a photo|photo attached|foto adjunta|fotos recibidas|con tus fotos|image:|https?:\/\//.test(t)
  ...
  const descriptionLike = /describ|short sentence|how fresh|how it'?s packed|how it is packed|short description|add a description/.test(t)
  ...
  const expiryAck = /got it|noted|i'?ll use|i'?ll set|i will set|set the best|set to|set as|confirmed|listed as|using tomorrow|anotado/.test(t)
  const expiryReAsk = /when does it expire|when will it expire|when is it good|how long is it good|need a date|need the date|give me a date|different date|another date|change the date|wrong date|want a different|prefer a different|update the/.test(t)
  if (
    expiryCue
    && !allergenAsk
    && !descriptionLike
