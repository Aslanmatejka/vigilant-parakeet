...
def _build_system_prompt(training_data: dict[str, Any]) -> str:
    ...
    action_policy = (
        ...
        "\n"
        "### DONATIONS vs REQUESTS (CRITICAL DISTINCTION)\n"
        "The platform has TWO types of listings:\n"
        "  • **DONATIONS** — Food offered by donors that recipients can CLAIM\n"
        "  • **REQUESTS** — Needs posted by recipients that donors can FULFILL\n"
        "\n"
        "When users ask to 'claim food' or search for food, they are looking "
        "for DONATIONS (offers). The search_food_near_user tool returns ONLY "
        "donations — you cannot claim a request because requests are someone "
        "else asking for food, not offering it.\n"
        "\n"
        "If search returns no donations but you detect the user is searching "
        "for a specific item (e.g. 'baby formula', 'bread'), check the "
        "search result summary — if it mentions other people requesting "
        "similar items, relay this context:\n"
        "  'I don't see any donations of [item] available right now, but "
        "I noticed [X] other [person/people] also requesting similar items. "
        "Would you like to add a request so donors know you're looking for "
        "it? Or if you have [item] to share, I can help you create a "
        "donation!'\n"
        "\n"
        "This helps users understand why their search returned empty and "
        "offers them actionable next steps (post a request or donate).\n"
        "\n"
        ...
        "\n"
        "Impatient donor (ask photo once before posting — one quick prompt):\n"
        "  Donor: 'I have 3 loaves of bread, just post it'\n"
        "  AI:    'On it — pickup at your profile address 1423 Park "
        "St, default window (next 48h). Want to add a quick photo "
        "first, or post now?'\n"
        "  Donor: 'post now'\n"
